...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot):
        ...
        # Resolve VOC2012 directory from datapath
        # Expected layouts:
        #  1) datapath=.../VOCdevkit (contains VOC2012/)
        #  2) datapath=... (contains VOCdevkit/VOC2012/)
        #  3) datapath=.../VOC2012
        voc2012_dir = None
        if os.path.basename(datapath.rstrip('/')) == 'VOC2012':
            voc2012_dir = datapath
        else:
            cand1 = os.path.join(datapath, 'VOC2012')
            cand2 = os.path.join(datapath, 'VOCdevkit', 'VOC2012')
            if os.path.exists(cand1):
                voc2012_dir = cand1
            elif os.path.exists(cand2):
                voc2012_dir = cand2
            else:
                # Fall back (kept for backward compatibility with the original PATNet repo layout)
                voc2012_dir = os.path.join('../VOCdevkit', 'VOC2012')

        self.img_path = os.path.join(voc2012_dir, 'JPEGImages')
        self.ann_path = os.path.join(voc2012_dir, 'SegmentationClassAug')


        # [patched] self.img_path is set below
        # [patched] self.ann_path is set below
